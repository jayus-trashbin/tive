...
const SessionDetailsModal: React.FC<Props> = ({ session, onClose }) => {
    ...
    return (
        <Modal 
            ...
            className="border-t sm:border border-white/10 p-0"
        >
            {/* Header */}
            <div
            ...
            >
                <div>
                    <div className="text-caption-xs font-bold text-zinc-500 uppercase tracking-widest mb-1 flex items-center gap-1">
                ...
                <button
                    onClick={onClose}
                    className="w-8 h-8 rounded-full bg-zinc-900 border border-zinc-800 flex items-center justify-center text-zinc-400 hover:text-white tap"
                >
                    <X size={18} />
                </button>
            </div>
            ...
            <div className="flex-1 p-6 space-y-8">
                        ...
                        <div className="grid grid-cols-3 gap-3">
                            <div className="bg-zinc-900 border border-white/5 rounded-2xl p-3 flex flex-col items-center justify-center gap-1">
                                ...
                                <span className="text-2xl font-bold text-white select-text tabular-nums">
                                    {session.volumeLoad > 1000 ? (session.volumeLoad / 1000).toFixed(1) + 'k' : session.volumeLoad}
                                </span>
                                <span className="text-caption-xs text-zinc-500 uppercase font-bold">Volume (kg)</span>
                                {/* A-03: delta badge */}
                                {totalComparison && (
                                    <span className={cn(
                                        "text-caption-xs font-bold flex items-center gap-0.5",
                            ...
                            <div className="bg-zinc-900 border border-white/5 rounded-2xl p-3 flex flex-col items-center justify-center gap-1">
                                ...
                                <span className="text-caption-xs text-zinc-500 uppercase font-bold">Duration</span>
                            </div>
                            <div className="bg-zinc-900 border border-white/5 rounded-2xl p-3 flex flex-col items-center justify-center gap-1">
                                <Trophy size={16} className="text-brand-warning" />
                                <span className="text-2xl font-bold text-white select-text tabular-nums">{session.sets.filter(s => s.isPR).length}</span>
                                <span className="text-caption-xs text-zinc-500 uppercase font-bold">Records</span>
                            </div>
                        </div>
                        ...
                        {previousSession && (
                            <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-3">
                                <div className="text-caption-xs font-bold text-zinc-500 uppercase tracking-wider mb-2 flex items-center gap-1">
                                    ...
                                    {Array.from(comparisonByExercise?.entries() || []).slice(0, 4).map(([exId, comp]) => {
                                        ...
                                        return (
                                            <div key={exId} className="bg-zinc-900 rounded-lg p-2">
                                                <div className="text-caption-xs text-zinc-400 font-bold truncate mb-1">{ex?.name || 'Exercise'}</div>
                                                <div className={cn("text-caption-xs font-bold flex items-center gap-0.5", volDiff >= 0 ? "text-brand-primary" : "text-red-400")}>
                                                    {volDiff >= 0 ? <TrendingUp size={8} /> : <TrendingDown size={8} />}
                                                    {volDiff >= 0 ? '+' : ''}{volDiff.toFixed(0)} kg vol
                                                </div>
                                                {comp.prevBest1RM > 0 && (
                                                    <div className={cn("text-caption-xs font-bold flex items-center gap-0.5", rmDiff > 0 ? "text-brand-primary" : rmDiff < 0 ? "text-red-400" : "text-zinc-500")}>
                                    ...
                                    <div key={i} className="animate-in slide-in-from-bottom-2 fade-in fill-mode-backwards" style={{ animationDelay: `${i * 50}ms` }}>
                                        <div className="flex items-center gap-3 mb-3">
                                            ...
                                            <div>
                                                ...
                                                <div className="text-caption-xs text-zinc-500 font-bold uppercase tracking-wider">{exercise?.targetMuscle}</div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="border border-white/5 rounded-2xl overflow-hidden bg-zinc-900/50">
                                            <div className="grid grid-cols-[30px_1fr_1fr_1fr] bg-zinc-900 border-b border-white/5 py-1.5 px-3">
                                                <div className="text-caption-xs font-bold text-zinc-600 text-center">#</div>
                                                <div className="text-caption-xs font-bold text-zinc-600 text-center">KG</div>
                                                <div className="text-caption-xs font-bold text-zinc-600 text-center">REPS</div>
                                                <div className="text-caption-xs font-bold text-zinc-600 text-center">1RM</div>
                                            </div>
                                            {sets.map((set, idx) => (
                                                <div key={set.id} className={cn(
                                                    ...
                                                    <div className="flex justify-center">
                                                        <span className={cn(
                                                            "w-5 h-5 rounded flex items-center justify-center text-caption-xs font-bold",
                                                            set.type === 'warmup' ? "bg-yellow-500/20 text-yellow-500" :
                                                                set.type === 'failure' ? "bg-red-500/20 text-red-500" :
                                                                    "bg-zinc-800 text-zinc-400"
                                                        )}>{idx + 1}</span>
                                                    </div>
                                                    <div className="text-center font-bold text-sm text-white select-text tabular-nums">{set.weight}</div>
                                                    <div className="text-center font-bold text-sm text-white select-text tabular-nums">{set.reps}</div>
                                                    <div className="text-center text-xs font-medium text-zinc-500 select-text tabular-nums">
                                                        {set.isPR ? (
                                        ...
                                        {note && (
                                            <div className="mt-2 flex items-start gap-2 px-3 py-2 bg-zinc-900/60 border border-zinc-800 rounded-xl">
                                                ...
                                                <p className="text-caption text-zinc-300 leading-relaxed">{note}</p>
                                            </div>
                                        )}
                                    </div>
                                );
                        ...
                        <div className="pt-8 flex justify-center" style={{ paddingBottom: `calc(env(safe-area-inset-bottom) + 2rem)` }}>
                            <button
                                ...
                                className="text-red-500/60 hover:text-red-500 text-xs font-bold uppercase tracking-widest flex items-center gap-2 transition-colors tap"
                            >
