    ...
    return (
        <form onSubmit={handleSubmit} className="flex-1 flex flex-col overflow-hidden">
                ...
                >
                                ...
                                <div className="flex bg-zinc-950 rounded-xl p-1 border border-zinc-800" role="radiogroup" aria-label="Select Gender">
                                    {(['male', 'female'] as const).map(g => (
                                        <button
                                            ...
                                            className={cn(
                                                "flex-1 py-3 rounded-lg text-xs font-bold capitalize transition-all",
                    ...
                    <section className="bg-zinc-900/30 border border-white/5 rounded-3xl p-5 space-y-4">
                        ...
                        <div className="space-y-4">
                            <div className="space-y-2">
                                ...
                                <div className="flex bg-zinc-950 rounded-xl p-1 border border-zinc-800" role="radiogroup" aria-label="Select Theme">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex bg-zinc-950 rounded-xl p-1 border border-zinc-800" role="radiogroup" aria-label="Select Unit System">
                                    {(['metric', 'imperial'] as const).map(u => (
                                        <button
                                            key={u}
                                            type="button"
                                            onClick={() => setFormData({ ...formData, unitSystem: u })}
                                            aria-pressed={formData.unitSystem === u}
                                            className={cn(
                                                "flex-1 py-3 rounded-lg text-xs font-bold capitalize transition-all",
                    ...
                    <div className="pt-4 border-t border-zinc-900">
                        <Button
                            ...
                            fullWidth
                            iconLeft={Trash2}
                            className="text-red-500/50 hover:text-red-500 border-none uppercase tracking-widest text-[10px]"
                            onClick={onReset}
                        >
            ...
            <div className="px-6 py-4 bg-zinc-950/90 backdrop-blur-md border-t border-white/5 shrink-0 pb-safe">
                <Button
                    variant={statusMsg?.type === 'error' ? 'danger' : statusMsg?.type === 'success' ? 'primary' : 'primary'}
                    type="submit"
                    size="lg"
                    fullWidth
                    iconLeft={statusMsg?.type === 'error' ? AlertCircle : statusMsg?.type === 'success' ? CheckCircle2 : Save}
                    className={cn(
                        "py-4 uppercase tracking-wider font-bold",
                        !statusMsg && "bg-white text-black hover:bg-zinc-200"
                    )}
                >
                    {statusMsg ? statusMsg.text : 'Save Changes'}
                </Button>
            </div>
        </form>
    );
