                ...
                <section>
                    ...
                    <div className="relative">
                        <textarea
                            ...
                            className="w-full h-64 bg-zinc-900/50 border border-zinc-800 text-white p-5 rounded-lg focus:outline-none focus:border-brand-primary/50 transition-colors font-medium text-sm placeholder:text-zinc-800 resize-none"
                        />
                        <div className="absolute bottom-4 right-4 flex gap-2">
                            <button
                                onClick={handleProcess}
                                disabled={!input.trim() || isProcessing}
                                className="flex items-center gap-2 px-6 py-3 bg-brand-primary text-black font-bold uppercase text-xs rounded-lg disabled:opacity-30 disabled:grayscale transition-all hover:scale-[1.02] active:scale-95 shadow-lg"
                            >
                                {isProcessing ? (
                        ...
                        >
                            ...
                            {isProcessing ? (
                                ...
                                <div className="space-y-4">
                                    {aiMessage && (
                                        <div className="p-4 bg-zinc-900 border border-zinc-800 text-zinc-400 text-xs font-medium leading-relaxed italic">
                                            {aiMessage}
                                        </div>
                                    )}
                                    ...
                                    {generatedRoutine && (
                                        <motion.div
                                            initial={{ scale: 0.98 }}
                                            animate={{ scale: 1 }}
                                            className="bg-zinc-950 border-2 border-brand-primary/20 rounded-lg overflow-hidden"
                                        >
                                            ...
                                            <div className="p-2">
                                                <button
                                                    ...
                                                    className="w-full py-4 bg-brand-primary text-black font-bold uppercase tracking-widest text-xs flex items-center justify-center gap-2 hover:bg-brand-accent transition-all shadow-lg"
                                                >
